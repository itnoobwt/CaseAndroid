...
public class MemoryCacheUtils
{
    ...
    public MemoryCacheUtils(){
        ...
        mMemorCache = new LruCache<String,Bitmap>((int)maxMemory){
            //用于计算每个条目的大小
            @Override
    ...
    public Bitmap getBitmapFromMemory(String url){
        ...
        if (bitmapSoftReference != null) {
            Bitmap bitmap = bitmapSo ftReference.get();
            return bitmap;
        }
        */
       return mMemorCache.get(url);
    }
    /**
     * 往内存中写图片
     * @param url
    ...
    public void clearCache(){
        if(mMemorCache != null)
            if(mMemorCache.size()>0)
                mMemorCache.evictAll();
    }
}
